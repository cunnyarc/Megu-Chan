...
class Dev(commands.Cog, name='Dev'):
    ...
    async def extension_group(self, ctx):
        await ctx.send("Usage: `>extension [command] [extension]`")
    ...
    async def reload_subcommand(self, ctx, extension):
        if extension == "all":
    ...
    @load_subcommand.error
    async def extension_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send("Usage: `>extension [command] [extension]`")

    @unload_subcommand.error
    async def extension_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send("Usage: `>extension [command] [extension]`")

    @reload_subcommand.error
    async def extension_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send("Usage: `>extension [command] [extension]`")
